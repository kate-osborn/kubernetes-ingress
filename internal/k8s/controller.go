...
func NewLoadBalancerController(input NewLoadBalancerControllerInput) *LoadBalancerController {
	...
	if input.CertManagerEnabled {
		opts := cm_controller.BuildOpts(lbc.ctx, lbc.restConfig, lbc.client, lbc.namespace)
		lbc.certManagerController = cm_controller.NewCmController(opts)
		// if err != nil {
		// 	glog.Fatalf("failed to create Spiffe Controller: %v", err)
		// }
	}
...
func (lbc *LoadBalancerController) Run() {
	...
	if lbc.certManagerController != nil {
		err := lbc.certManagerController.Run(2, lbc.ctx.Done())
		if err != nil {
			glog.Fatal(err)
		}
	}
